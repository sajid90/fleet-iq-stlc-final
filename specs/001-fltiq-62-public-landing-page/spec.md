...
| Story §Scope + design source | TR-003 | Scenario 1, Scenario 5 | TC-005, TC-028, TC-044, TC-045 | Automated — TC-028 passing; **TC-005 FAILING (two Product Defects: the product mark's square icon contains no "F" glyph, and it scrolls to the top only on the first click of a page load)**; **TC-044/045 FAILING (Product Defect: header renders no "Capabilities"/"Hierarchy" anchor links at all)** |
...
| Story §Scope + design source | TR-006 | Scenario 5 | TC-029, TC-030 | Automated — TC-030 passing; **TC-029 FAILING (Product Defect: hero subhead/stats copy diverges from spec)** |
...
| unbracketed-5 + design source | TR-008 | Scenario 3 | TC-014, TC-015, TC-016, TC-017, TC-018, TC-019, TC-020, TC-021 | Automated — only TC-016 passing; **TC-014/015/017/018/019/020/021 FAILING (Product Defect: hierarchy explorer is static markup with no detail panel or selection behaviour implemented)** |
| Story §Scope + design source | TR-009 | Scenario 1 | TC-033 | `test_landing.py::test_tc033_*` — Automated, **FAILING (Product Defect: closing-section body uses "You'll", not spec's "You will")** |
...
| Story §Scope + design source | TR-019 | Scenario 5 | TC-032 | `test_landing.py::test_tc032_*` — Automated, passing |
...
| [FLTIQ-60] | TR-015 | Scenario 5 | TC-035 (partial automation + manual, see plan.md A3) | Not automated — `automatable: false` in test-cases.json; stays fully manual (plan.md A3) |
