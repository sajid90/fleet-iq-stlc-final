...
| 2026-09-21 | Correction | §14's Automation column updated with execution evidence from `/speckit-implement`, and again after TC-005/TC-006 were extended to cover a repeat scroll-and-click cycle. Both now fail, exposing a Product Defect: the header product mark (TR-003) and footer "Back to top" (TR-010) scroll to the top only on the first click of a page load — once the URL fragment is `#top`, further clicks are same-fragment navigations the app no-ops. A control experiment on a plain `<a href="#top">` page re-scrolled every time, so this is the app breaking standard browser behaviour, not an over-reading of the requirement. | No `TR-xxx` wording changed — TR-003/TR-010 already state the behaviour with no first-click-only qualifier, so this is coverage of existing text plus execution evidence. No new `TC-xxx` was created; the two existing cases were extended. `test-cases.json`, `tasks.md` and their Change Logs updated the same day. | Stayed Approved |
| 2026-09-21 | Correction | §14's Automation column updated after a design-source cross-check found the build ships "Create an account" everywhere TR-003/TR-006/TR-009 (and the design) say "Create account" — six instances across header, hero and closing section. TC-028/030/033 were tightened from a case-insensitive substring order check to an exact, case-sensitive comparison of the labels themselves, which is what caught it; the substring form had been passing on a wrong label since the suite was first written. | No `TR-xxx` wording changed — the exact text was already there. `test-cases.json`, `tasks.md` and their Change Logs updated the same day; folded into defect **D5**. | Stayed Approved |
| 2026-09-22 | Correction | TR-008 amended to add the hierarchy panel's own header line ("XYZ — organisation"), which the original §15a enumeration correctly found but recorded as "not itself a requirement, subject of the still-open §13b Q4" — Q4 had actually closed 2026-09-21 ("today's exact static values are what ships now"), but nothing acted on that closure for this specific element, and §11's prose was never updated to say so. Caught a full day later by a requester screenshot comparison against the design, not by this spec's own review. §11 gained a dated correction note (append-only — the original paragraph is left as written) rather than a silent rewrite. TC-032 (previously TR-019 only) extended to assert this line; TR-008's Source column gained `decision` alongside the design source. | TC-032 now **fails**: the build renders "XYZ" only, missing "— organisation" entirely. New defect **D10** (`reports/test-report-cycle-1.md`). `data-model.md` gained a field for this line. `test-cases.json`/`tasks.md` updated the same day. | Stayed Approved |
| 2026-09-22 | Correction | §14's TR-015 row updated: TC-035 moved from fully manual to partially automated (task T038, previously skipped on a bookkeeping error — see `tasks.md` Change Log, same date). The automated half found defect **D9**: 5 of 8 checked surfaces render the wrong Keel design-system token (backgrounds), and capability cards/hierarchy panel/header are missing their border, radius and shadow. Constitution amended to 1.3.0 in the same session (principle VII.a — static UI text is asserted exactly, never semantically), prompted by this and two earlier findings (the CTA label above, the "F" icon glyph, TR-003/TR-010) that had all shipped as passing tests behind lenient assertions. | No `TR-xxx` wording changed. `test-cases.json` (`automatable`/`automation_status` corrected), `tasks.md` (T038 checked, new Change Log rows) and `CLAUDE.md`/`speckit-implement` SKILL.md (exact-match rule for future work) updated the same day. | Stayed Approved |
...
| TR-008 | Selecting a node in the hierarchy explorer updates the detail panel (label, status badge, device count, device type, config source, note) without a page reload and without any call to a FleetIQ API; on first render, before any visitor interaction, "Floor 1" is pre-selected. Each tree row also displays its own right-aligned text independent of the detail panel — for Building A and Building B this is a **subgroup count** ("2 subgroups" / "0 subgroups"), not a device count, even though the detail panel's "Devices" field for those same nodes shows 79 and 0 respectively. The selected node's own tree row is also visually distinguished from the other rows (a background highlight and a left-edge accent), independent of and in addition to the detail panel updating. The panel's own header line above the tree reads the static value "XYZ — organisation" verbatim. | unbracketed-5; default-selection behaviour, the tree-row/detail-panel field distinction, and the row's own selected-state styling (`rowStyle`, driven by `node.isSelected`/`aria-pressed`) all confirmed verbatim in the design source (`state = { selected: 'a1' }`; the `meta` field is separate from `devices` — see data-model.md). The panel header line was design-sourced but excluded pending §13b Q4 (permanent vs. placeholder) until the requester's 2026-09-21 decision closed it: "today's exact static values are what ships now" — see §13b, §11 correction note. | scope-and-story-acceptance + presentation-and-interaction + decision | DEFINED | Functional | P1 | Medium |
...
|---------|-------------|-------------|------------|------------|
| unbracketed-1 | TR-001 | Scenario 1 | TC-001 | `test_landing.py::test_tc001_*` — Automated, passing |
| unbracketed-4 | TR-002 | Scenario 2 | TC-009 | `test_landing.py::test_tc009_*` — Automated, conditional-skip (research.md R5; no test account configured) |
| Story §Scope + design source | TR-003 | Scenario 1, Scenario 5 | TC-005, TC-028, TC-044, TC-045 | Automated — **TC-005 FAILING (two Product Defects: the product mark's square icon contains no "F" glyph, and it scrolls to the top only on the first click of a page load)**; **TC-028 FAILING (Product Defect: header CTA reads "Create an account", not "Create account" — D5)**; **TC-044/045 FAILING (Product Defect: header renders no "Capabilities"/"Hierarchy" anchor links at all)** |
| unbracketed-2 | TR-004 | Scenario 1 | TC-002, TC-003, TC-004, TC-007 | `test_landing.py::test_tc00{2,3,4,7}_*` — Automated, all passing |
| unbracketed-3 | TR-005 | Scenario 2 | TC-010, TC-011, TC-012 | `test_landing.py::test_tc01{0,1,2}_*` — Automated, all passing |
| Story §Scope + design source | TR-006 | Scenario 5 | TC-029, TC-030 | Automated — **TC-029 FAILING (Product Defect: hero subhead/stats copy diverges from spec)**; **TC-030 FAILING (Product Defect: hero CTA reads "Create an account", not "Create account" — D5)** |
| Story §Scope + design source | TR-007 | Scenario 5 | TC-031 | `test_landing.py::test_tc031_*` — Automated, **FAILING (Product Defect: capability cards 02/03/06 use contractions, not spec's exact wording)** |
| unbracketed-5 + design source + decision | TR-008 | Scenario 3, Scenario 5 | TC-014, TC-015, TC-016, TC-017, TC-018, TC-019, TC-020, TC-021, TC-032 | Automated — only TC-016 passing; **TC-014/015/017/018/019/020/021 FAILING (Product Defect: hierarchy explorer is static markup with no detail panel or selection behaviour implemented)**; **TC-032 FAILING (Product Defect: panel header reads "XYZ", missing "— organisation" — D10)** |
| Story §Scope + design source | TR-009 | Scenario 1 | TC-033 | `test_landing.py::test_tc033_*` — Automated, **FAILING (two Product Defects: closing-section body uses "You'll", not spec's "You will"; and its CTA reads "Create an account", not "Create account" — D5)** |
| Story §Scope + design source | TR-010 | Scenario 1, Scenario 5 | TC-006, TC-008, TC-034 | Automated — TC-008 passing; **TC-006 FAILING (Product Defect: "Back to top" scrolls to the top only on the first click of a page load — same root cause as TC-005)**; **TC-034 FAILING (two Product Defects: footer has no "©ACL Digital" attribution text, and its square mark icon contains no "F" glyph)** |
| Story §Scope + design source | TR-019 | Scenario 5 | TC-032 | `test_landing.py::test_tc032_*` — Automated. TR-019's own heading/intro assertions pass; **TC-032 as a whole FAILS on its added TR-008 org-label check (D10)** — see the TR-008 row above |
| [TIA-AC-74] | TR-011 | Scenario 4 | TC-021, TC-022, TC-041 | Automated — TC-022/041 passing; TC-021 fails only because it shares TR-008's detail-panel gap |
| [TIA-AC-77] | TR-012 | Scenario 4 | TC-023, TC-024, TC-027 | Automated — TC-023/024 passing; **TC-027 FAILING (Product Defect: hierarchy nodes have no keyboard/ARIA semantics — same root cause as TR-008)** |
| [TIA-AC-80] | TR-013 | Scenario 4 | TC-025 | `test_landing.py::test_tc025_*` (axe-core) — Automated, **FAILING (Product Defect: 9 serious color-contrast violations)** |
| [TIA-AC-81] | TR-014 | Scenario 4 | TC-026, TC-041 | `test_landing.py::test_tc026_*` — Automated, passing |
| [FLTIQ-60] | TR-015 | Scenario 5 | TC-035 (partial automation + manual, see plan.md A3) | Partially automated 2026-09-22 — `test_landing.py::test_tc035_*`, **FAILING (Product Defect D9: 5 of 8 checked surfaces render the wrong design-system token; cards/panel/header missing border, radius, shadow)**; pixel-fidelity half stays manual (plan.md A3) |
| unbracketed-6/7 | TR-016 | Scenario 5 | TC-036 | `test_landing.py::test_tc036_*` — Automated, passing |
| Story §Scope (out of scope) | TR-017 | Scenario 5 | TC-037, TC-038 | `test_landing.py::test_tc03{7,8}_*` — Automated, both passing |
| design source + requester decision, 2026-09-21 | TR-018 | Scenario 5 | TC-039 | `test_landing.py::test_tc039_*` — Automated, passing |
| requester decision, 2026-09-21 | TR-020 | Scenario 1, Scenario 2 | TC-042 | `test_landing.py::test_tc042_*` — Automated, **FAILING (Product Defect: no skeleton loader observed during auth resolution; see tasks.md/Completion Report for the test-technique caveat)** |

*(TC-043/EC-003 stays Blocked — no automation task exists by design, constitution II. See the Completion Report for the full defect list and pass/fail counts.)*
...
**Correction, 2026-09-22**: the paragraph above is stale and was not updated
when §13b Q4 closed on 2026-09-21 (see §13b) — left as originally written per
this section's append-only discipline rather than silently edited. Q4 is
**not** still open: the requester's decision that day was "today's exact
static values are what ships now," which resolves the org-label line the
same as every other hierarchy example value. It is now folded into TR-008
above. The gap this correction fixes: nothing acted on Q4's closure for this
specific element at the time, so it stayed both untested and mis-cited as
open for a full day — caught only by a requester screenshot comparison
against the design, not by this spec's own review.
