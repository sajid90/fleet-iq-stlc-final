...
| Total test cases | 43 |
| P1 / P2 / P3 | 22 / 16 / 5 |
| Automated (target) | 42 (TC-035 partially automated 2026-09-22 — see T038) |
...
| 2026-09-22 | Correction | TC-035 corrected from `automatable: false`/`automation_status: "Manual"` to `true`/`"Automated"`, and task T038 implemented, after a requester screenshot comparison caught a visibly grey capabilities-grid block the suite had no test for. The original `automatable: false` treated a bookkeeping flag as a technical limit: comparing rendered computed styles against the Keel tokens the design source declares is fully deterministic, and T038 already specified this exact audit -- it had simply never been built. Manual visual-regression review (plan.md A3) is unaffected; only the deterministic half moved from unautomated to automated. | New defect **D9**: 5 of 8 checked surfaces (capabilities grid, capability card, hierarchy panel, closing section, footer) render the wrong token, and cards/panel/header are missing border, radius and shadow. Logged in `reports/test-report-cycle-1.md`; verdict escalated to name D9 as a blocking defect. `spec.md` §14, Test Case Summary and Traceability table (this file) updated: Automated 41 -> 42. | No re-review needed -- `test-cases.json`'s own `automatable` flag was corrected, not a `TR-xxx` |
| 2026-09-21 | Correction (coverage gap, not a spec change) | TC-028, TC-030 and TC-033 (CTA order checks) tightened from a case-insensitive substring match (`"create" in t.lower()`) to an exact, case-sensitive, ordered list comparison, after a design-source cross-check found the build ships "Create an account" where the design says "Create account" and the substring form let it pass unnoticed on all six instances of the label. No new `TC-xxx` -- TR-003/006/009 already required this exact text; the assertion was simply too lenient to catch a wrong one. | TC-028/030/033 now **fail** on the label itself, not just order. Folded into defect **D5** (`reports/test-report-cycle-1.md`), which grew from 6 to 7 copy strings. `spec.md` §14 updated. | No re-review needed -- no `TR-xxx` text changed; this is an assertion-precision fix plus a defect finding |
| 2026-09-22 | Correction (coverage gap, not a spec change) | T035/TC-032 extended to assert the hierarchy panel's own header line ("XYZ — organisation", TR-008), after a requester screenshot comparison caught it was untested. The gap traces back further than the missing test: `spec.md` §11 had classified this element as "not itself a requirement, subject of the still-open §13b Q4" -- but Q4 actually closed 2026-09-21 ("today's exact static values are what ships now"), and nothing acted on that closure for this specific line. It stayed both untested and mis-cited as open for a full day, caught only by inspection, not by this file's own review. TR-008 amended in `spec.md` to add the line; TC-032's `requirements` array gained TR-008 alongside TR-019. | TC-032 now **fails**: the build renders "XYZ" only. New defect **D10**. `data-model.md` gained a field entry for this line so it is no longer undocumented. `spec.md` §11 gained a dated correction note (append-only) rather than a rewrite. | No re-review needed -- `TR-008`'s addition is a citation of design content already resolved by an existing decision, not a new product decision |
| 2026-09-22 | Correction | Constitution amended to 1.3.0, adding principle VII.a: static UI text (buttons, links, headings, static copy) is asserted with exact, case-sensitive `==` against the correct element's own isolated text -- never a substring, lowercased, or regex match used to judge correctness. Prompted by three findings in this cycle hidden behind lenient assertions (the CTA label above, the "F" icon glyph, and the "tenant owner" case). Applied retroactively to the two remaining lenient checks found in this suite: TC-017's `nodes[id]["meta"] in <whole row text>` became an exact match against the isolated meta span (new `hierarchy_row_meta_value()`, which strips the row's own "· " separator -- a rendering artifact, not part of the data-model.md `meta` field); TC-034's `"FleetIQ" in <mark text>` and `"ACL Digital" in <whole footer text>` became exact matches, the latter via a new `footer_attribution_text()` that isolates the attribution element instead of comparing against the footer's concatenated text. TC-036/TC-038's case-insensitive checks were reviewed and left unchanged -- they assert the *absence* of banned terms, where case-insensitive matching is the correct choice (any casing of a banned term is still a violation), not a case of judging whether present text is *correct*. | No test's pass/fail outcome changed: TC-017 and TC-034 already failed on other defects (D1, D8) and continue to; the tightened assertions are more precise about *why*, not a new result. `CLAUDE.md` and `.claude/skills/speckit-implement/SKILL.md` updated with the same rule for future implementation work. | No re-review needed -- no `TR-xxx`/`TC-xxx` content changed, only assertion rigor |
...
- [x] T035 [P] [S5] Automate the hierarchy section's exact heading and intro-paragraph copy assertion in `automation/tests/ui/test_landing.py` (covers: TC-032) — heading/intro pass; **extended 2026-09-22 to also cover TR-008's panel header line, which now FAILS (Product Defect D10: renders "XYZ", missing "— organisation")**
...
- [x] T038 [P] [S5] Automate the partial design-system token/component usage audit for TR-015 (covers: TC-035) — **implemented 2026-09-22, FAILING: Product Defect D9.** `test-cases.json` was corrected first (`automatable: true`, `automation_status: "Automated"`) since the earlier `false`/`Manual` flag was a bookkeeping error, not a technical limit: comparing computed styles against the Keel tokens the design source declares is fully deterministic. The audit checks 8 surfaces' backgrounds plus border/radius/shadow on cards and the hierarchy panel; 5 of 8 surfaces and both border/radius/shadow checks fail. See D9 in `reports/test-report-cycle-1.md`. The manual visual-regression half (pixel fidelity, spacing, typography rendering) stays manual per plan.md A3.
...
|-------|-------|----------|------------|
| TC-035 | Every visual element resolves to a shared design-system token or component | P2 | Pixel-level fidelity to the Keel design isn't a boolean a single Playwright assertion can cleanly cover (spec.md §11); T038 automates the component/token-usage half (implemented 2026-09-22, failing — D9), a human visual-regression review (plan.md A3, 2h) covers the rest |
...
| TR-008 | S3, S5 | TC-014, TC-015, TC-016, TC-017, TC-018, TC-019, TC-020, TC-021, TC-032 | T019, T020, T021, T022, T023, T024, T025, T035 | `automation/tests/ui/test_landing.py` |
...
| TR-015 | S5 | TC-035 | T038 (partial, implemented 2026-09-22) + manual (plan.md A3) | `automation/tests/ui/test_landing.py` |
